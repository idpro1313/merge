...
- [ ] Every important function/component has a CONTRACT (PURPOSE, INPUTS, OUTPUTS) inside `START_METHOD_*` … `END_METHOD_*` when governed code uses outer wrappers
- [ ] Non-trivial classes use paired `START_CLASS_*` / `END_CLASS_*` when governed code uses outer wrappers
- [ ] START_METHOD / END_METHOD and START_CLASS / END_CLASS markers are paired when present
