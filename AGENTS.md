...
Maintenance note: this template is synthesized from GRACE package rules, references, and artifact templates. In installed IDE packages, resolve `GRACE_PACKAGE_ROOT` as `.cursor/grace/`, `.kilo/grace/`, or `.opencode/grace/`; in the canonical source package, use the package root. When core rules change, update this template in the same pass so project onboarding stays aligned with the canonical package.

## Keywords
merge-tap, game, Telegram, merge, 2048-like

## Annotation
Merge Tap — игра-головоломка на слияние предметов в Telegram Mini App. Backend на Express + sql.js, frontend на React 18, Telegram Bot API.

## Core Principles

### 1. Never Write Code Without a Contract
Before generating or editing any module, create or update its file-level MODULE_CONTRACT with PURPOSE, SCOPE, DEPENDS и LINKS. Function contracts carry PURPOSE, INPUTS, OUTPUTS, SIDE_EFFECTS и LINKS; strict/high-risk fields are optional per `rules/grace-code-markup.md`. Plan-level module contracts carry public inputs, outputs, and errors. The contract is the source of truth. Code implements the contract, not the other way around.

### 2. Semantic Markup Is Load-Bearing Structure
Markers like `START_CLASS_*` / `END_CLASS_*`, `START_METHOD_*` / `END_METHOD_*`, and `START_BLOCK_*` / `END_BLOCK_*` are navigation anchors, not documentation. Outer **METHOD**/**CLASS** pairs bound whole symbols for chunking; **BLOCK** pairs segment logic inside the body. They must be:
- uniquely named
- paired
...
Testing, traces, and log anchors are designed before large execution waves. `docs/verification-plan.xml` is part of the architecture, not an afterthought. Logs are evidence: on critical paths they must reveal **which branches executed** (mutually exclusive arms, early returns, error paths—not only the happy path), using stable markers shared between production and tests. Tests are executable contracts. Canonical detail: `rules/grace-logging.md`, `references/verification-driven-development.md`.
...
//   LINKS: [Ссылки на knowledge graph]
...
### Class and method wrappers (outer layer)
```
...
//   SIDE_EFFECTS: [Изменения внешнего состояния или "нет"]
...
```
Use `START_CLASS_<Name>` … `END_CLASS_<Name>` around entire classes the same way. See `rules/grace-code-markup.md`.
...
The core file-level `MODULE_CONTRACT` remains `PURPOSE`, `SCOPE`, `DEPENDS` и `LINKS`; `ROLE` and `MAP_MODE` are optional lint metadata.
...
- instrument **every behavior-changing branch** (including else arms, early returns, and error paths) so logs reconstruct **which path ran** in prod and tests; mutually exclusive arms must be **distinguishable** via markers or stable structured fields
...
- use strict LDD markers (`[CLASSIFIER][IMP:1-10][FUNCTION][BLOCK][OPERATION]`) for critical or high-autonomy flows
...
- for each important branch (success, failure, alternate path), define scenarios and required/forbidden markers in `docs/verification-plan.xml`; markers in code must match prod and test collectors
